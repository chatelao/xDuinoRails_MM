  ...
  if (sscanf(line, "cv %d %d", &val1, &val2) == 2) {
    ...
    if (line[1] == ' ' && line[2] != '\0') {
      ...
      if (sub == 'p') {
        ...
        DEBUG_SERIAL.print("Serial: High Speed CSV Logging ");
        DEBUG_SERIAL.println(logger.isHighSpeedEnabled() ? "ON" : "OFF");
      }
    } else {
...
void SerialConsole::printHelp() {
  ...
  DEBUG_SERIAL.println("L [p/w/c/b/h]  : Toggle logging (p: protocol, w: pwm, c: cv, b: bemf, h: highspeed)");
